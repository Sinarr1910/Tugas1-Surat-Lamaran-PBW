...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nama_mahasiswa && $nama_produk && $nama_pegawai && $jumlah_beli > 0 && $tanggal_transaksi) {
        $stmt = $conn->prepare("INSERT INTO transaksi (nama_mahasiswa, nama_produk, nama_pegawai, jumlah_beli, tanggal_transaksi) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("sssid", $nama_mahasiswa, $nama_produk, $nama_pegawai, $jumlah_beli, $tanggal_transaksi);

        if ($stmt->execute()) {
            $_SESSION['notif']      = 'Transaksi berhasil ditambahkan!';
            $_SESSION['notif_type'] = 'success';
        } else {
            $_SESSION['notif']      = 'Gagal menambahkan transaksi.';
    ...
    <div class="card p-4">
        ...
        <form method="POST" action="tambah.php">
            <div class="mb-3">
                ...
                <input type="text" name="nama_mahasiswa" class="form-control" required maxlength="100">
            </div>
            <div class="mb-3">
